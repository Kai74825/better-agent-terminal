...
- 2026-05-11：修正 WorkerPanel Procfile log filter 誤觸。process name 不再隱含切換 spotlight，改成明確的 focus log icon button，避免使用者點啟動/切換 process 時誤把其他 process log 從 terminal view 過濾掉；Procfile runtime 後續可評估把 process supervision/log mux 從 renderer 移到 Rust native。
