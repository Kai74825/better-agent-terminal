...
最近一次更新：2026-05-09。（最新一輪：把 `notification.*` in-memory store（list/markRead/markAllRead/markWindowRead/clear/focusLatestUnread/focusEntry/onUpdate）port 到 Rust，採 `Mutex<Vec<NotificationEntry>>` + `Emitter::emit("notification:update")` 推 push 訊息；外加 `system.onResume` 在 Tauri 下回 no-op unsub；renderer 的 notification-store 全切到 `host.notification.*`；cargo test 67 測例。）
...
- [~] 把全部 `window.batAppAPI.*` 直呼換成 `host.*`：已 port 命名空間（settings、shell、dialog、fs、clipboard、image、pty、workspace、update、debug、git、app、notification、system）都已切到 `host.*`；剩下未 port 的命名空間（claude、codex、openai、worktree、profile、snippet、github、agent、claudeCli、claudeAccount、remote、tunnel、worker）仍走 `window.batAppAPI` 直到該命名空間有 Rust 對應或 Node sidecar 接管。
