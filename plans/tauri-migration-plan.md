...
- [x] **Tests**
  - `tests/host-api.test.ts`：8 個情境，第 3 個 invoke 情境涵蓋 67+ 條 cmd（settings、shell、dialog、fs、clipboard、image、pty、workspace、update、debug、git、app、notification、github、snippet、profile），含 optional title、camelCase 參數、undefined args（picker no-arg invokes）、optional 參數的 undefined 透傳（`commit_hash`、`workspaceId` 等）。
  - `tests/tauri-launch.test.ts`：啟動 release exe 3 秒，斷言沒提前崩。
  - `cargo test`（89 tests）：每個 commands::* 子模組都有 unit test。包含：`settings::tests`（路徑 + `resolve_shell_path` 的 unix/windows 分支）、`shell::tests`、`dialog::tests`、`fs::tests`（讀檔、>512 KiB cap、不存在路徑、ignored dirs、tilde expansion、entry sort、readdir 跳過 IGNORED、search 深度與筆數上限、mkdir 名稱驗證、deletePath 只接受目錄）、`path_guard::tests`（deny-list 命中、目錄包含、白名單）、`clipboard::tests`、`image::tests`（mime 對應、PNG round-trip、>10 MiB cap）、`pty::tests`（id validation、shell selection 的 unix/windows 分支）、`workspace::tests`、`update::tests`、`debug::tests`、`git::tests`（URL 解析、log/diff/status 的 parsing、argv builder、count clamping、cwd validation）、`app::tests`（single-window 常數、camelCase serde、openNewInstance 行為）、`notification::tests`（normalize_workspace_key、camelCase serde、mark/clear 不變式、50-entry cap、workspace dedupe、focus_latest_unread 篩選）、`github::tests`（json-or-error fallback、CliStatus serde、cwd validation）、`snippet::tests`（migration backfill、query lowercase 對稱、sort 順序、workspace 過濾、delete 語意、tempfile round-trip）、`profile::tests`（default 常數、camelCase serde、create echo、reject mutations）。
- [x] **Release build verified on Windows** — `pnpm exec tauri build` 產生 ~12 MB exe + ~5 MB MSI + ~3.5 MB NSIS installer，smoke test 通過。
...
- [ ] **Phase 2：Agent SDK Node sidecar**（見下方 [Phase 2 設計筆記] 章節）。剩下未 port 的命名空間 claude、openai、agent、worker、worktree 都綁在 agent runtime / SDK；計畫採 stdio + JSON-RPC 包一個 Node sidecar，由 Tauri spawn 並維持 lifetime。
